...
export class CalculCaloriesComponent implements OnInit {
  ...
  conseils: PlatConseils[] = [
    {
      taille: Taille.Maigre,
      maxQuantite: null,
      minQauntite: null,
      conseil: '',
    },
    {
      taille: Taille.Correct,
      maxQuantite: null,
      minQauntite: null,
      conseil: '',
    },
  ...
  ngOnInit(): void {
    ...
    if (this.isLoggedIn) {
      ...
      this.newPlat.creatorId = this.ownerId; 
      this.newnewPlat.creatorId = this.ownerId;
  ...
  GetAdvice(id: any) {
    ...
    this.conseilAdapted =
      platTrouve?.platConseils?.find((c) => c.taille === yourTaille) ??
      this.conseilEmpty;
    this.poids = null;
    this.age = null;
    this.genre = null;
  ...
  addPlat(): void {
    ...
    this.platService
      .addPlat({ plat: this.newPlat, conseil: this.conseils })
      .subscribe((plat) => {
  ...
  addPlatQuantite() {
    ...
    this.platsQuantites.platsQuantity = [
      ...(this.platsQuantites.platsQuantity || []),
      platQuantite,
    ];
  ...
  getPlatNameById(platId: number): string {
    const plat = this.plats.find(
      (p) => p.idPlat === parseInt(platId.toString())
    );
