...
export class AddToCartComponent implements OnInit, AfterViewInit {
  ...
  constructor(private storageService: StorageService, private cartService: CartService, private productsService: ProductsService) {}
    ...
    if (this.cartItems.length > 0) {
      this.cartItems.forEach(item => {
        if (item.idProducts !== undefined && item.quantity !== undefined) {
          this.productsService.sellProduct(item.idProducts, item.quantity).subscribe(
            () => {
          ...
          console.error('idProducts or quantity is undefined for the cart item:', item);
        }
      });
    } else {
